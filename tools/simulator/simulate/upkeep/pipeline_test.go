...
import (
	...
	"github.com/goplugin/plugin-automaton/tools/simulator/util"
)
...
func TestCheckPipeline(t *testing.T) {
	...
	broadcaster := chain.NewBlockBroadcaster(conf.Blocks, 1, logger, nil, loadUpkeepAt(upkeep1, 2))
...
func TestIsEligible(t *testing.T) {
	...
	for i := range tests {
		eligible := isEligible(tests[i].eligibles, tests[i].performs, tests[i].block)
