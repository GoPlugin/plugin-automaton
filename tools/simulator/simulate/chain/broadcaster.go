...
const (
	progressTelemetryNamespace = "Broadcasting simulated blocks"
)
...
func NewBlockBroadcaster(conf config.Blocks, maxDelay int, logger *log.Logger, progress ProgressTelemetry, loaders ...BlockLoaderFunc) *BlockBroadcaster {
	...
	total := conf.Duration + conf.EndPadding

	if progress != nil {
		_ = progress.Register(progressTelemetryNamespace, int64(total))
	}
...
func (bb *BlockBroadcaster) broadcast() {
	...
	if bb.progress != nil {
		bb.progress.Increment(progressTelemetryNamespace, 1)
	}
